...
#        b_state = 1 # self.gpio.digitalRead(self.b_pin)
        r_state = a_state 
        return r_state
...
#        print (r_seq, self.r_seq)
        if r_seq != self.r_seq:
#            delta = (r_seq - self.r_seq) % 2
            if  (r_seq - self.r_seq >0):
               delta = 1
            else:
               delta =0
 ...
 #       self.gpio.wiringPiISR(self.b_pin, self.gpio.INT_EDGE_BOTH, isr)
    ...
    class Worker(threading.Thread):
        def __init__(self, pins):
            ...
            self.stopping = False
            self.encoder = SingleRotaryEncoder(pins)
            self.daemon = True
            self.delta = 0
            self.delay = 0.0005
            self.speed = 0
